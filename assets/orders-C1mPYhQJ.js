import{a as E}from"./sidebar-84I8JWhY.js";import{s as C}from"./toast-ZwFb22xq.js";import{a as B}from"./auth-CC3zXfFT.js";import{s as T,r as D}from"./reusableModals-f8FhhYVB.js";const l=document.getElementById("page-content"),$=document.getElementById("modal-root");$||console.error("Modal root not found");function G(a){const s={pending:{label:"معلقة",color:"bg-yellow-100 text-yellow-800 border-yellow-200"},in_progress:{label:"قيد التنفيذ",color:"bg-blue-100 text-blue-800 border-blue-200"},done_unpaid:{label:"تمت ولم يُدفع",color:"bg-orange-100 text-orange-800 border-orange-200"},done_paid:{label:"تمت ودُفع",color:"bg-green-100 text-green-800 border-green-200"},cancelled:{label:"ملغاة",color:"bg-red-100 text-red-800 border-red-200"}}[a]||{label:a,color:"bg-gray-100 text-gray-800 border-gray-200"};return`<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${s.color}">${s.label}</span>`}function m(a){return`EGP ${Number(a||0).toLocaleString("ar-EG")}`}function R(a,n,s,d=[]){if(!a.length)return{tableRows:`
        <tr>
      ...
      `};const c=a.map(e=>{const u=n.find(t=>t.client_id===e.client_id)||{},h=s.find(t=>t.chalet_id===e.chalet_id)||{},y=e.completed_at||(["done_paid","done_unpaid","cancelled"].includes(e.status)?new Date().toISOString().split("T")[0]:"-"),b=d.filter(t=>!t.is_deleted&&t.order_id===e.order_id&&t.type==="expense").reduce((t,f)=>t+Number(f.amount||0),0),w=d.filter(t=>!t.is_deleted&&t.order_id===e.order_id&&t.type==="income").reduce((t,f)=>t+Number(f.amount||0),0),S=(w>0?w:Number(e.deposit||0))-b;return`
        <tr class="hover:bg-slate-700/60">
          ...
          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-100">${m(e.price)}</td>
          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-100">${m(e.deposit)}</td>
          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-100">${m(b)}</td>
          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-100">${m(S)}</td>
          ...
          <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-400">${y}</td>
        ...
        <div class="md:hidden bg-slate-800 rounded-3xl border border-slate-700 p-4 mb-4">
          <div class="flex items-start justify-between mb-3">
            <div>
              ...
              <p class="text-sm text-slate-400">${u.name||"غير محدد"} - ${h.chalet_name||"غير محدد"}</p>
            </div>
            ${G(e.status)}
          </div>
            ...
            <div class="flex justify-between items-center">
              ...
              <span class="text-lg font-semibold text-slate-50">${m(e.price)}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-sm text-slate-400">الدفعة:</span>
              <span class="text-sm text-slate-50">${m(e.deposit)}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-sm text-slate-400">المصروفات:</span>
              <span class="text-sm text-slate-50">${m(b)}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-sm text-slate-400">صافي الربح:</span>
              <span class="text-sm text-slate-50">${m(S)}</span>
            </div>
            <div class="flex justify-between items-center">
            ...
            <div class="flex justify-between items-center">
              ...
              <span class="text-sm text-slate-50">${y}</span>
            </div>
      ...
      `}).join("");return{tableRows:c,mobileCards:p}}async function H(a,n){return $?new Promise(s=>{const d=`
      <div class="space-y-4">
    ...
    `;D($,"تأكيد حذف الطلب",d);const c=$.querySelector("#cancel-delete-button"),p=$.querySelector("#confirm-delete-button");c==null||c.addEventListener("click",()=>{$.innerHTML="",s(!1)}),p==null||p.addEventListener("click",async()=>{try{await E.deleteOrder(a),C("success","تم حذف الطلب بنجاح"),$.innerHTML="",n(),s(!0)}catch(e){console.error("Error deleting order:",e),C("error","حدث خطأ أثناء حذف الطلب"),s(!1)}})}):(console.error("Modal root not available for delete confirmation"),!1)}function A(a,n,s){return a.filter(d=>{const c=n.find(y=>y.client_id===d.client_id)||{},p=`${d.order_id} ${d.notes||""} ${d.status} ${c.name}`.toLowerCase(),e=s.search?p.includes(s.search.toLowerCase()):!0,u=s.status?d.status===s.status:!0,h=s.client?d.client_id===s.client:!0;return e&&u&&h})}async function U(a,n,s){await T(a,n,s)}async function L(){if(!B.isAuthenticated()){window.location.href="login.html";return}if(!l)return;const[a,n,s]=await Promise.all([E.getOrders(),E.getClients(),E.getChalets()]),d=await E.getTransactions(),c={orders:a.length,clients:n.length,chalets:s.length,revenue:a.reduce((i,o)=>i+Number(o.price||0),0),totalNetProfit:d.filter(i=>!i.is_deleted).reduce((i,o)=>i+(o.type==="income"?Number(o.amount||0):-Number(o.amount||0)),0)};l.innerHTML=`
    <div class="p-6 max-w-[1200px] mx-auto px-4 space-y-6">
      ...
      <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div class="card card-padded">
          ...
          <p class="mt-3 text-3xl font-semibold text-slate-50">${c.orders}</p>
        </div>
        <div class="card card-padded">
          <p class="text-sm text-slate-400">عدد العملاء</p>
          <p class="mt-3 text-3xl font-semibold text-slate-50">${c.clients}</p>
        </div>
        <div class="card card-padded">
          <p class="text-sm text-slate-400">عدد الشاليهات</p>
          <p class="mt-3 text-3xl font-semibold text-slate-50">${c.chalets}</p>
        </div>
        <div class="card card-padded">
          <p class="text-sm text-slate-400">إجمالي الإيراد</p>
          <p class="mt-3 text-3xl font-semibold text-slate-50">${m(c.revenue)}</p>
        </div>
        <div class="card card-padded">
          <p class="text-sm text-slate-400">صافي الربح الكلي</p>
          <p class="mt-3 text-3xl font-semibold text-slate-50">${m(c.totalNetProfit)}</p>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select id="order-client-filter" class="form-select">
              ...
              ${n.map(i=>`<option value="${i.client_id}">${i.name}</option>`).join("")}
            </select>
          </div>
        </div>
      </div>
  ...
  `;const p=l.querySelector("#order-search"),e=l.querySelector("#order-status-filter"),u=l.querySelector("#order-client-filter"),h=l.querySelector("#orders-table-body"),y=l.querySelector("#orders-mobile-body"),b=l.querySelector("#add-order-button"),w=l.querySelector("#total-pending"),I=l.querySelector("#total-done_unpaid"),S=l.querySelector("#total-done_paid"),t=l.querySelector("#total-all"),f=l.querySelector("#total-expenses"),O=l.querySelector("#total-net-profit");function k(){const i={search:p.value.trim(),status:e.value,client:u.value},o=A(a,n,i),{tableRows:_,mobileCards:g}=R(o,n,s,d);h.innerHTML=_,y.innerHTML=g;const r={pending:0,done_unpaid:0,done_paid:0,all:0,expenses:0,netProfit:0};o.forEach(v=>{const P=Number(v.price||0);v.status==="pending"?r.pending+=P:v.status==="done_unpaid"?r.done_unpaid+=P:v.status==="done_paid"&&(r.done_paid+=P),r.all+=P;const j=d.filter(x=>!x.is_deleted&&x.order_id===v.order_id&&x.type==="expense").reduce((x,N)=>x+Number(N.amount||0),0),q=d.filter(x=>!x.is_deleted&&x.order_id===v.order_id&&x.type==="income").reduce((x,N)=>x+Number(N.amount||0),0),M=(q>0?q:Number(v.deposit||0))-j;r.expenses+=j,r.netProfit+=M}),w.textContent=r.pending.toLocaleString("ar-EG"),I.textContent=r.done_unpaid.toLocaleString("ar-EG"),S.textContent=r.done_paid.toLocaleString("ar-EG"),t.textContent=r.all.toLocaleString("ar-EG"),f.textContent=r.expenses.toLocaleString("ar-EG"),O.textContent=r.netProfit.toLocaleString("ar-EG")}b==null||b.addEventListener("click",()=>{U(n,s,L)}),l.dataset.ordersEventsBound=l.dataset.ordersEventsBound||"",l.dataset.ordersEventsBound||(document.addEventListener("click",async i=>{const o=i.target.closest('button[data-action="delete-order"]');if(o){const g=o.dataset.orderId;g&&await H(g,L);return}const _=i.target.closest('button[data-action="edit-order"]');if(_){const g=_.dataset.orderId;if(!g)return;const r=a.find(v=>v.order_id===g);if(!r)return;await T(n,s,L,r);return}}),document.addEventListener("change",async i=>{const o=i.target.closest("select[data-order-id]");if(!o)return;const _=o.dataset.orderId,g=o.value;if(_)try{await E.updateOrder(_,{status:g}),C("success","تم تحديث حالة الطلب بنجاح"),L()}catch{C("error","خطأ في تحديث الطلب"),o.value=o.dataset.currentStatus}}),l.dataset.ordersEventsBound="true"),p==null||p.addEventListener("input",k),e==null||e.addEventListener("change",k),u==null||u.addEventListener("change",k),k()}window.location.pathname.includes("orders.html")&&document.addEventListener("DOMContentLoaded",L);export{L as r};
