import{a as x,s as y}from"./toast-DJo7gZdP.js";const Y={clients:[],chalets:[],orders:[]},Q="malaz-cleaning",X=`https://${Q}-default-rtdb.firebaseio.com`,k={clients:null,chalets:null,orders:null,transactions:null};async function w(e,t="GET",n=null){try{const a=x.getToken();let l=`${X}${e}.json`;a&&(l+=`?auth=${a}`);const s={method:t,headers:{"Content-Type":"application/json"}};n&&(s.body=JSON.stringify(n));const r=await fetch(l,s);if(!r.ok)throw r.status===401?(x.logout(),new Error("انتهت صلاحية الجلسة، يرجى تسجيل الدخول مرة أخرى")):new Error(`Firebase error: ${r.statusText}`);return r.json()}catch(a){throw console.error("Firebase Error:",a),a}}async function $(e){if(k[e])return k[e];try{const t=await w(`/${e}`);return t?(k[e]=Object.values(t).filter(n=>!n.is_deleted),k[e]):(k[e]=[],[])}catch(t){return console.warn(`Failed to fetch ${e} from Firebase, using local state:`,t),k[e]=Y[e]||[],k[e]}}function g(...e){e.forEach(t=>{k[t]!==void 0&&(k[t]=null)})}async function U(e){try{return await w(`/${e}`)||{}}catch(t){return console.warn(`Failed to fetch raw ${e} from Firebase:`,t),{}}}function B(e,t,n=3){const l=Object.keys(e||{}).filter(s=>typeof s=="string").reduce((s,r)=>{if(!r.startsWith(t))return s;const c=parseInt(r.slice(t.length),10);return Number.isNaN(c)?s:Math.max(s,c)},0);return`${t}${String(l+1).padStart(n,"0")}`}const S={async getClients(){return $("clients")},async getChalets(){return $("chalets")},async getOrders(){return $("orders")},async addClient({type:e,name:t,phone:n}){const a=await $("clients"),l=`CL${String(a.length+1).padStart(3,"0")}`,s={client_id:l,type:e,name:t,phone:n,created_at:new Date().toISOString().split("T")[0],is_deleted:!1};return await w(`/clients/${l}`,"PUT",s),g("clients"),s},async addChalet({client_id:e,chalet_name:t,location:n,details:a}){const l=await $("chalets"),s=`CH${String(l.length+1).padStart(3,"0")}`,r={chalet_id:s,chalet_code:s,client_id:e,chalet_name:t,location:n,details:a,created_at:new Date().toISOString().split("T")[0],is_deleted:!1};return await w(`/chalets/${s}`,"PUT",r),g("chalets"),r},async addOrder({client_id:e,chalet_id:t,status:n,price:a,notes:l,created_at:s,scheduled_at:r="",deposit:c=0,created_by:o=""}){const f=await U("orders"),i=B(f,"OR",3),p={order_id:i,client_id:e,chalet_id:t,status:n,price:Number(a),notes:l,created_at:s,scheduled_at:r||"",deposit:Number(c||0),created_by:o||"",completed_at:n.includes("done")?s:"",is_deleted:!1};await w(`/orders/${i}`,"PUT",p),g("orders");try{if(p.deposit>0&&await this.addTransaction({type:"income",amount:Number(p.deposit),date:p.created_at,details:`دفعة مقدمة من الطلب ${i}`,order_id:i,created_by:p.created_by||""}),p.status==="done_paid"){const m=Number(p.price||0)-Number(p.deposit||0);m>0&&await this.addTransaction({type:"income",amount:m,date:p.created_at,details:`باقي الدفع من الطلب ${i}`,order_id:i,created_by:p.created_by||""})}}catch(m){console.error("Failed to create income transaction for order:",m)}return p},async updateClient(e,t){return await w(`/clients/${e}`,"PATCH",t),g("clients"),(await $("clients")).find(a=>a.client_id===e)},async updateChalet(e,t){return await w(`/chalets/${e}`,"PATCH",t),g("chalets"),(await $("chalets")).find(a=>a.chalet_id===e)},async updateOrder(e,t){const a=(await $("orders")).find(r=>r.order_id===e)||{};if(t.status){const r=new Date().toISOString().split("T")[0];t.status==="done_paid"||t.status==="done_unpaid"||t.status==="cancelled"?t.completed_at=r:(t.status==="pending"||t.status==="in_progress")&&(t.completed_at="")}await w(`/orders/${e}`,"PATCH",t),g("orders");const s=(await $("orders")).find(r=>r.order_id===e);try{const r=a.status,c=t.status||(s==null?void 0:s.status),o=Number(t.price??(s==null?void 0:s.price)??0),f=Number(t.deposit??(s==null?void 0:s.deposit)??0),p=(await this.getTransactions()).filter(m=>!m.is_deleted&&m.order_id===e&&m.type==="income").reduce((m,h)=>m+Number(h.amount||0),0);if(f>0&&f>Number(a.deposit||0)){const m=f-Number(a.deposit||0);m>0&&await this.addTransaction({type:"income",amount:m,date:new Date().toISOString().split("T")[0],details:`زيادة الدفعة من الطلب ${e}`,order_id:e,created_by:t.created_by||(s==null?void 0:s.created_by)||""})}if(r!=="done_paid"&&c==="done_paid"){const h=(await this.getTransactions()).filter(u=>!u.is_deleted&&u.order_id===e&&u.type==="income").reduce((u,b)=>u+Number(b.amount||0),0),d=o-h;if(d>0){const u=t.created_by||(s==null?void 0:s.created_by)||"";await this.addTransaction({type:"income",amount:d,date:new Date().toISOString().split("T")[0],details:`باقي الدفع من الطلب ${e}`,order_id:e,created_by:u})}}}catch(r){console.error("Failed to create income transaction for updated order:",r)}return s},async deleteClient(e){const t=await w(`/clients/${e}`,"PATCH",{is_deleted:!0});return g("clients"),t},async deleteChalet(e){const t=await w(`/chalets/${e}`,"PATCH",{is_deleted:!0});return g("chalets"),t},async deleteOrder(e){const t=await w(`/orders/${e}`,"PATCH",{is_deleted:!0});return g("orders"),t},async getTransactions(){return $("transactions")},async getWorkerAttendance(e){const t=e==="august"?"august":"july";try{return await w(`/worker_attendance/${t}`)||{workers:[]}}catch(n){return console.warn(`Failed to fetch worker attendance for ${t}:`,n),{workers:[]}}},async saveWorkerAttendance(e,t){const n=e==="august"?"august":"july",a={...t,month:n,updated_at:new Date().toISOString().split("T")[0]};return await w(`/worker_attendance/${n}`,"PUT",a),a},async addTransaction({type:e,amount:t,date:n,details:a="",order_id:l="",created_by:s=""}){const r=await U("transactions"),c=B(r,"TR",4),o={transaction_id:c,type:e,amount:Number(t||0),date:n||new Date().toISOString().split("T")[0],details:a||"",order_id:l||"",created_by:s||"",created_at:new Date().toISOString(),is_deleted:!1};return await w(`/transactions/${c}`,"PUT",o),g("transactions"),o},async updateTransaction(e,t){return await w(`/transactions/${e}`,"PATCH",t),g("transactions"),(await $("transactions")).find(a=>a.transaction_id===e)},async deleteTransaction(e){const t=await w(`/transactions/${e}`,"PATCH",{is_deleted:!0});return g("transactions"),t}};function T(e,t,n){if(!e)return null;const a=document.createElement("div");a.className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4",a.innerHTML=`
  ...
  `,e.appendChild(a);const l=a.querySelector('button[aria-label="Close"]');function s(){a.remove()}return a.addEventListener("click",r=>{r.target===a&&s()}),l==null||l.addEventListener("click",s),a}function j(e,t,n="",a=""){return`
    <div>
  ...
  `}function L(e,t,n,a="",l="",s=""){const r=n.map(c=>{const o=c.value===a?"selected":"";return`<option value="${c.value}" ${o}>${c.label}</option>`}).join("");return`
    <div>
      <label class="form-label" for="${e}">${t}</label>
  ...
  `}function D(e="grid-cols-1 md:grid-cols-2",t=""){return`<div class="grid ${e} gap-4">${t}</div>`}function te(e,t,n="primary"){return`<button type="button" class="${n==="secondary"?"btn btn-secondary":"btn btn-primary"}" id="${e}">${t}</button>`}function ae(e=[]){return`<div class="flex flex-col sm:flex-row justify-end gap-3 pt-4">${e.map(n=>te(n.id,n.label,n.style)).join(" ")}</div>`}function M(e=[],t=[]){const n=e.join(""),a=ae(t);return`<div class="space-y-4">${n}${a}</div>`}function C(e,t={}){const n={};for(const[a,l]of Object.entries(t)){const s=e.querySelector(`#${l}`);s&&(n[a]=s.value)}return n}function q(e,t={}){for(const n of Object.values(t)){const a=e.querySelector(`#${n}`);a&&(a.type==="number"?a.value="":a.tagName==="SELECT"?a.selectedIndex=0:a.value="")}}const ne=[{value:"owner",label:"owner"},{value:"broker",label:"broker"}];function se(e={}){return[j("client-name","الاسم","اسم العميل",e.name||""),Z("client-phone","الهاتف","رقم الهاتف",e.phone||""),W("client-type","النوع",ne,e.type||"owner")]}function re(e=[],t={}){const n=e.map(a=>({value:a.client_id,label:a.name}));return[j("chalet-name","الشاليه","اسم الشاليه",t.chalet_name||""),j("chalet-location","الموقع","الموقع الجغرافي",t.location||""),L("chalet-client","العميل",n,t.client_id||"","add-new-client-btn","+ عميل"),G("chalet-details","التفاصيل","تفاصيل الشاليه",4,t.details||"")]}function le(e=[],t=[],n={}){var c;const a=e.map(o=>({value:o.client_id,label:o.name})),l=n.client_id||((c=e[0])==null?void 0:c.client_id)||"",s=t.filter(o=>o.client_id===l).map(o=>({value:o.chalet_id,label:o.chalet_name})),r=[{value:"pending",label:"معلقة"},{value:"in_progress",label:"قيد التنفيذ"},{value:"done_unpaid",label:"تمت ولم يُدفع"},{value:"done_paid",label:"تمت ودُفع"},{value:"cancelled",label:"ملغاة"}];return[L("order-client","العميل",a,l,"add-client-btn","+ عميل"),L("order-chalet","الشاليه",s,n.chalet_id||"","add-chalet-btn","+ شاليه"),D("grid-cols-1 md:grid-cols-2",W("order-status","الحالة",r,n.status||"pending")+V("order-price","السعر","مثلاً 420",n.price||"")),D("grid-cols-1 md:grid-cols-2",ee("order-scheduled","تاريخ التنفيذ",n.scheduled_at||"")+V("order-deposit","الديبوزيت","مثلاً 100",n.deposit||"")),G("order-notes","الملاحظات","تفاصيل إضافية",3,n.notes||"")]}function oe(e={}){const t=se(e);return M(t,[{id:"save-client-button",label:"حفظ العميل"}])}function ce(e=[],t={}){const n=re(e,t);return M(n,[{id:"save-chalet-button",label:"حفظ الشاليه"}])}function ie(e=[],t=[],n={}){const a=le(e,t,n);return M(a,[{id:"save-order-button",label:"حفظ الطلب"}])}const _={notEmpty:(e,t)=>e!=null&&e.trim()?null:`${t} مطلوب`,phone:(e,t="الهاتف")=>e!=null&&e.trim()?/^(\+|00)?[0-9]{7,15}$/.test(e.replace(/\s/g,""))?null:`رقم ${t} غير صحيح`:`${t} مطلوب`,number:(e,t,n=!1)=>{if(e===""||e===null)return`${t} مطلوب`;const a=Number(e);return isNaN(a)?`${t} يجب أن يكون رقم`:!n&&a<=0?`${t} يجب أن يكون أكبر من 0`:null},email:(e,t="البريد الإلكتروني")=>e!=null&&e.trim()?/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)?null:`${t} غير صحيح`:`${t} مطلوب`};function I(e,t){const n={};for(const[a,l]of Object.entries(e))if(t[a]){const s=t[a](l);s&&(n[a]=s)}return n}function O(e){return Object.keys(e).length>0}function H(e){const t=Object.keys(e)[0];return e[t]}async function J(e=null,t=null){const n=document.getElementById("modal-root"),a=oe(t||{}),l=T(n,t?"تعديل عميل":"إضافة عميل جديد",a);if(!l)return;function s(){l.remove()}const r=l.querySelector("#save-client-button");r==null||r.addEventListener("click",async()=>{const c=C(l,{name:"client-name",phone:"client-phone",type:"client-type"}),f=I(c,{name:i=>_.notEmpty(i,"الاسم"),phone:i=>_.phone(i,"الهاتف"),type:i=>_.notEmpty(i,"النوع")});if(O(f)){y("error",H(f));return}try{if(t&&t.client_id){const i=await S.updateClient(t.client_id,c);y("success","تم تعديل بيانات العميل"),s(),e&&e(i)}else{const i=await S.addClient(c);y("success","تم إضافة العميل بنجاح"),q(l,{name:"client-name",phone:"client-phone",type:"client-type"}),s(),e&&e(i)}}catch{y("error","حدث خطأ أثناء حفظ بيانات العميل")}})}async function de(e=[],t=null,n=null){const a=document.getElementById("modal-root"),l=ce(e,n||{}),s=T(a,n?"تعديل شاليه":"إضافة شاليه جديد",l);if(!s)return;function r(){s.remove()}const c=s.querySelector("#chalet-client");s.querySelector("#chalet-name");const o=s.querySelector("#add-new-client-btn"),f=s.querySelector("#save-chalet-button");o==null||o.addEventListener("click",async()=>{await J(async i=>{const p=c.value;c.innerHTML+=`<option value="${i.client_id}">${i.name}</option>`,c.value=i.client_id})}),f==null||f.addEventListener("click",async()=>{const i=C(s,{chalet_name:"chalet-name",location:"chalet-location",client_id:"chalet-client",details:"chalet-details"}),m=I(i,{chalet_name:h=>_.notEmpty(h,"اسم الشاليه"),location:h=>_.notEmpty(h,"الموقع"),client_id:h=>_.notEmpty(h,"العميل")});if(O(m)){y("error",H(m));return}try{if(n&&n.chalet_id){const h=await S.updateChalet(n.chalet_id,i);y("success","تم تعديل الشاليه"),r(),t&&t(h)}else{const h=await S.addChalet(i);y("success","تم إضافة الشاليه بنجاح"),q(s,{chalet_name:"chalet-name",location:"chalet-location",client_id:"chalet-client",details:"chalet-details"}),r(),t&&t(h)}}catch{y("error","خطأ في حفظ الشاليه")}})}function pe(e={},t={}){const n=document.getElementById("modal-root");if(!n||!e)return;const a=`
  ...
  `;T(n,"تفاصيل الشاليه",a)}async function me(e=[],t=[],n=null,a=null){const l=document.getElementById("modal-root"),s=ie(e,t,a||{}),r=T(l,a?"تعديل طلب":"إضافة طلب جديد",s);if(!r)return;function c(){r.remove()}const o=r.querySelector("#order-client"),f=r.querySelector("#order-chalet"),i=r.querySelector("#add-client-btn"),p=r.querySelector("#add-chalet-btn"),m=r.querySelector("#save-order-button");function h(){const d=o.value,u=t.filter(b=>b.client_id===d);f.innerHTML=u.length?u.map(b=>`<option value="${b.chalet_id}">${b.chalet_name}</option>`).join(""):'<option value="">لا يوجد شاليهات</option>'}o==null||o.addEventListener("change",h),a&&(o.value=a.client_id||o.value,h(),f.value=a.chalet_id||f.value,r.querySelector("#order-status").value=a.status||r.querySelector("#order-status").value,r.querySelector("#order-price").value=a.price||r.querySelector("#order-price").value,r.querySelector("#order-notes").value=a.notes||r.querySelector("#order-notes").value,r.querySelector("#order-scheduled")&&(r.querySelector("#order-scheduled").value=a.scheduled_at||""),r.querySelector("#order-deposit")&&(r.querySelector("#order-deposit").value=a.deposit||0)),i==null||i.addEventListener("click",async()=>{await J(async d=>{e.push(d);const u=o.value;o.innerHTML+=`<option value="${d.client_id}">${d.name}</option>`,o.value=d.client_id,h()})}),p==null||p.addEventListener("click",async()=>{await de(e,async d=>{t.push(d);const u=o.value;d.client_id===u&&(f.innerHTML+=`<option value="${d.chalet_id}">${d.chalet_name}</option>`)})}),m==null||m.addEventListener("click",async()=>{const d=C(r,{client_id:"order-client",chalet_id:"order-chalet",status:"order-status",price:"order-price",notes:"order-notes",scheduled_at:"order-scheduled",deposit:"order-deposit"});d.deposit=Number(d.deposit||0);const b=I(d,{client_id:v=>_.notEmpty(v,"العميل"),chalet_id:v=>_.notEmpty(v,"الشاليه"),price:v=>_.number(v,"السعر"),status:v=>_.notEmpty(v,"الحالة")});if(O(b)){y("error",H(b));return}try{if(a&&a.order_id){const v={client_id:d.client_id,chalet_id:d.chalet_id,status:d.status,price:Number(d.price),notes:d.notes,scheduled_at:d.scheduled_at||"",deposit:Number(d.deposit||0)};await S.updateOrder(a.order_id,v),y("success","تم تحديث الطلب"),c(),n&&n()}else d.created_at=new Date().toISOString().split("T")[0],d.created_by=x.getUserName(),await S.addOrder({...d,price:Number(d.price)}),y("success","تم إضافة الطلب بنجاح"),q(r,{client_id:"order-client",chalet_id:"order-chalet",status:"order-status",price:"order-price",notes:"order-notes"}),c(),n&&n()}catch(v){console.error(v),y("error","خطأ في حفظ الطلب")}})}async function fe(e=[],t=[],n=null,a=null){var h,d;const l=document.getElementById("modal-root");if(!l)return;const s=`
  ...
  `,r=await S.getClients(),c=new Map(t.map(u=>[u.chalet_id,u])),o=new Map(r.map(u=>[u.client_id,u.name])),f='<option value="">عام</option>'+(e.length?e.map(u=>{const b=c.get(u.chalet_id),v=(b==null?void 0:b.chalet_name)||u.chalet_id||"غير محدد",E=b&&o.get(b.client_id)||"غير محدد";return`<option value="${u.order_id}" ${(a==null?void 0:a.order_id)===u.order_id?"selected":""}>${u.order_id} - ${v} (${E})</option>`}).join(""):""),i=`
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <select id="tx-order" class="form-select">${f}</select>
      </div>
  ...
  `,p=T(l,a?"تعديل المعاملة":"إضافة معاملة جديدة",i);if(!p)return;function m(){p.remove()}(h=p.querySelector("#cancel-tx"))==null||h.addEventListener("click",()=>{m()}),(d=p.querySelector("#save-tx"))==null||d.addEventListener("click",async()=>{var F,R,A,P,z;const u=(F=p.querySelector("#tx-type"))==null?void 0:F.value,b=Number(((R=p.querySelector("#tx-amount"))==null?void 0:R.value)||0),v=(A=p.querySelector("#tx-date"))==null?void 0:A.value,E=((P=p.querySelector("#tx-order"))==null?void 0:P.value)||"",N=((z=p.querySelector("#tx-details"))==null?void 0:z.value)||"";if(!u||!b){y("error","الرجاء تعبئة النوع والمبلغ");return}try{a&&a.transaction_id?(await S.updateTransaction(a.transaction_id,{type:u,amount:b,date:v,order_id:E,details:N}),y("success","تم تعديل المعاملة")):(await S.addTransaction({type:u,amount:b,date:v,order_id:E,details:N,created_by:x.getUserName()}),y("success","تم إضافة المعاملة")),m(),n&&n()}catch(K){console.error("Transaction save error",K),y("error","حدث خطأ أثناء حفظ المعاملة")}})}function he(e){if(!e)return;e.innerHTML=`
